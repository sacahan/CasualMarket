...
class TWStockResponse(BaseModel):
    ...
    bid_prices: list[float] = Field(default_factory=list, description="買價 (五檔)")
    bid_volumes: list[int] = Field(default_factory=list, description="買量 (五檔)")
    ask_prices: list[float] = Field(default_factory=list, description="賣價 (五檔)")
    ask_volumes: list[int] = Field(default_factory=list, description="賣量 (五檔)")
    update_time: datetime = Field(..., description="更新時間")
    last_trade_time: str = Field(..., description="最後交易時間")

    @field_validator("symbol")
    def validate_symbol(cls, v):
        """驗證股票代號格式（支援 4-6 位數字加可選字母）。"""
        import re

        if not re.match(r"^\d{4,6}[A-Z]*$", v.upper()):
            raise ValueError("股票代號格式不正確，必須是 4-6 位數字加可選字母")
        return v
    ...
    @field_serializer("update_time", mode="plain")
    def _serialize_update_time(self, v: datetime):
        return v.isoformat()
...
class TWAPIRawResponse(BaseModel):
    ...
    msgArray: list[dict] = Field(default_factory=list, description="股票資料陣列")
    ...
    queryTime: dict = Field(default_factory=dict, description="查詢時間資訊")
