...
class SecuritiesDatabase:
    ...
    def _validate_database(self) -> None:
        ...
        try:
            ...
            raise ValueError(f"資料庫檔案無效: {e}") from e

    def find_by_stock_code(self, stock_code: str) -> SecurityRecord | None:
        """
        根據股票代碼查詢證券記錄。
        ...
        try:
            ...
            raise ValueError(f"資料庫查詢錯誤: {e}") from e
...
def get_securities_database() -> SecuritiesDatabase | None:
    """
    取得證券資料庫實例。
...
def resolve_stock_symbol(query: str) -> str | None:
    """
    解析股票查詢為標準股票代碼。
