...
test_uvx_execution() {
  ...
  echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}' | timeout 3s uvx --from "$PROJECT_ROOT" casual-market-mcp >/dev/null 2>&1 && {
    echo "✅ uvx 基本執行測試通過"
    return 0
  } || {
...
test_mcp_protocol() {
  ...
  echo "$init_request" | timeout 5s uvx --from "$PROJECT_ROOT" casual-market-mcp 2>/dev/null | {
    read -r response
...
verify_project_structure() {
  ...
  if ! grep -q "casual-market-mcp" "$PROJECT_ROOT/pyproject.toml"; then
    echo "❌ pyproject.toml 中找不到 casual-market-mcp 腳本入口點"
    return 1
  fi

  # Check main module exists
  if [[ ! -f "$PROJECT_ROOT/src/main.py" ]]; then
    echo "❌ 找不到 src/main.py"
    return 1
  fi
...
main() {
  ...
  echo "   command: uvx"
  echo "   args: [\"--from\", \"$PROJECT_ROOT\", \"casual-market-mcp\"]"
}
